...
public class Profiler {
	...
	//
	private static Hashtable _events;

	private static boolean _isEnabled;
	private static boolean _isMemoryEnabled;
		
	static {
		_threads = new Hashtable(50);
		_events = new Hashtable(50);
		_isEnabled = true;
	 ...
	 */
	public static final void enable(boolean isEnabled) {
	 ...
	 * @param writer Writer.
	 */
	public static final void printStackTrace(PrintWriter writer) {
		if (!_isEnabled) {
			return;
		}
	...
	public static final void print(PrintWriter writer) {
		...
		while (profiles.hasNext()) {
			...
			if (_isMemoryEnabled) {	
				msg.append("\n");
				msg.append("  memory consumed = " + 
					eventProfile.getTotalConsumedMemory() + " (bytes)");
				msg.append("  memory collected = " + 
					eventProfile.getTotalCollectedMemory() + " (bytes)");
			}
	...
	protected static final Stack getThreadStack() {
		...
		Stack stack = (Stack)_threads.get(threadName);
		if (stack == null) {
			stack = new Stack();
			_threads.put(threadName, stack);
		}
	...
	protected static final EventProfile getEventProfile(String event) {
		...
		EventProfile profile = (EventProfile)_events.get(event);
		if (profile == null) {
			profile = new EventProfile(event);
			_events.put(event, profile);
		}
